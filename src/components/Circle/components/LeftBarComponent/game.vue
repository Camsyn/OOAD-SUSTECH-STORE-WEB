<template>
  <div v-if="GameList" id="main">

    <div style="padding: 10px">
      <v-card
        class="mx-auto"
        v-for="(item,i) in GameList.slice((currentPage-1)*PageSize,currentPage*PageSize)"
        :key="i"
      >
        <v-col>
          <v-avatar>
...
export default {
  ...
  computed:{
    GameList(){
      this.totalCount=circle.state.dynamicPool.filter(item=>item.tag==="game").length
