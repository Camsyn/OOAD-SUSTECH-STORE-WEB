<template>
  <div id="main">
    ...
    <div v-if="TeasingList" style="padding: 10px">
      <v-card
        class="mx-auto"
        v-for="(item,i) in TeasingList.slice((currentPage-1)*PageSize,currentPage*PageSize)"
        :key="i"
      >
        <v-col>
          <v-avatar>
...
export default {
  ...
  computed:{
    TeasingList(){
      this.totalCount=circle.state.dynamicPool.filter(item=>item.tag==="teasing").length
      return circle.state.dynamicPool.filter(item=>item.tag==="teasing")
    }
  }
};
</script>
