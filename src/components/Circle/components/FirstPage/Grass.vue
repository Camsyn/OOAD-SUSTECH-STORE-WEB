...
export default {
  ...
  methods:{
    handleClick(item){
      this.$store.commit('GrassClick1',{item})
    },
    handleClick2(item){
      this.$store.commit('GrassClick2',{item})
    },
    handleClick3(item){
      this.$store.commit('GrassClick3',{item})
    },
    reply_click(item){
      this.$store.commit('Grassreply_click',{item})
    }
  },
  computed:{
    GrassList(){
      return circle.state.grassList
    }
  }
};
</script>
